...
class _SearchDetailGetPageState extends State<SearchDetailGetPage> {
            ...
            if (snapshot.connectionState == ConnectionState.waiting){
              ...
              return Scaffold(
                ...
                appBar: AppBar(
                  ...
                  actions: <Widget>[
                    PopupMenuButton(
                      icon: Icon(Icons.add_box, color: appColor.shade700,),
                      onSelected: (value) {
                                                                  ...
                                                                  TextButton(
                                                                    onPressed: () {
                                                                      ...
                                                                      Navigator.pushReplacement(
                                                                        context,
                                                                        MaterialPageRoute(
                                                                          builder: (BuildContext context) => MainView(), //추후 업데이트 할 것
                                                                        ),
                                                                      );
                                                                    },
  ...
  Future savePerfomance(homeIsbn) async{ //메모 post
    final response = await http.post(
      Uri.parse(server + '/bookmemo/save?isbn='+'${homeIsbn}'),
  ...
  Future<String> _postData(String selectedScreen, String homeIsbn) async { //정보 업데이트 함수
    ...
    try {
      if (selectedScreen == '읽은') {
        ...
        final response = await http.post(
          Uri.parse(server + '/book/changeall?isbn=' + '${homeIsbn}'),
          headers: <String, String>{
            'Content-Type': 'application/json',
            'Authorization': 'Bearer ${loginController.idToken}'
          },
...
Future<Map<String, dynamic>> _getISBN(homeIsbn) async { //상세정보 가져오는 함수
  ...
  final response = await client.get(Uri.parse(server + '/book/detail?isbn='+'${homeIsbn}'),
      headers: <String, String>{
        'Authorization': 'Bearer ${loginController.idToken}'
      });
...
Future<void> deleteBook(BuildContext context, homeIsbn) async {
  http.Client client = http.Client();
  final response = await client.delete(Uri.parse(server + '/book/delete?isbn='+'${homeIsbn}'),
      headers: <String, String>{
        'Authorization': 'Bearer ${loginController.idToken}'
      });
  // 특정 작업 수행
...
Future<void> updateMemo(BuildContext context, content, page, memoId) async {
  ...
  print(memoId);
  http.Client client = http.Client();
  final response = await client.post(Uri.parse(server + '/bookmemo/update/$memoId'),
      headers: <String, String>{
        'Authorization': 'Bearer ${loginController.idToken}',
...
Future<void> deleteMemo(BuildContext context, memoId, homeIsbn) async {
  ...
  http.Client client = http.Client();
  final response = await client.delete(Uri.parse(server + '/bookmemo/delete/$memoId'),
      headers: <String, String>{
        'Authorization': 'Bearer ${loginController.idToken}'
      });
